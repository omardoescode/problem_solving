...
// an optimization is to have bitsets to watch the state of rows, diagonals, and left diagonals occupied
bitset<30> rw, ld, rd;
...
void backtrack(int c) {
    ...
    for (int r = 0; r < 8; r++) {
        if (rw[r] || ld[r - c + 8 - 1] || rd[r + c]) continue;
    ...
    while (t--) {
        ...
        for (int k = 0; k < 92; k++)
            if (solutions[k][b] == a) {
                ...
                for (int j = 1; j < 8; ++j) printf(" %d", solutions[k][j] + 1);
                printf("\n");
            }
